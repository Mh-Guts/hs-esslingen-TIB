...
public:
	//Konstruktor mit den Default-Parametern
	Customer(std::string name = "", int age = 0, std::string location = "")
		: _name(name), _location(location), _age(age), _business_done(0), _transaction_count(0), _id(++_s_id_generator) {
		_s_count++;
	}
	...
	//Methode zum dürchführen der Geschäfte 
	void do_business(double amount) {
		_business_done += amount;
		_transaction_count++;
	}

	// Statische Methode zur Rückgabe der Anzahl der Kunden
